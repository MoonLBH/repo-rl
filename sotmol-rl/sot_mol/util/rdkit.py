...
def _compute_single_mol_strain_energy(mol, use_heavy_atom_norm=True):
    """
    Compute conformational strain energy for one RDKit mol with a 3D conformer.

    Definition:
        E_strain = E(initial conformer) - E(minimized conformer)

    Returns
    -------
    dict with:
        strain_energy: float or None
        strain_energy_per_atom: float or None
        ff_name: str or None
    """
    if mol is None:
        return {"strain_energy": None, "strain_energy_per_atom": None, "ff_name": None}
    ...
    try:
        ...
        # 可按你的体系决定是否加氢
        # 力场通常对显式H更稳一些
        mol_h = Chem.AddHs(mol_work, addCoords=True)
        ...
        if mmff_props is not None:
            ff = AllChem.MMFFGetMoleculeForceField(mol_h, mmff_props, confId=0)
            ff_name = "MMFF94"
        ...
        e_init = float(ff.CalcEnergy())

        # 重新构造一个力场对象用于最小化，避免状态污染
        mol_min = Chem.Mol(mol_h)
        if ff_name == "MMFF94":
            mmff_props_min = AllChem.MMFFGetMoleculeProperties(mol_min, mmffVariant='MMFF94')
            ff_min = AllChem.MMFFGetMoleculeForceField(mol_min, mmff_props_min, confId=0)
        else:
            ff_min = AllChem.UFFGetMoleculeForceField(mol_min, confId=0)

        ff_min.Minimize()
        e_min = float(ff_min.CalcEnergy())

        strain = e_init - e_min
        if strain < 0:
            # 数值误差下可能略小于0
            strain = max(strain, 0.0)

        n_atoms = mol.GetNumHeavyAtoms() if use_heavy_atom_norm else mol.GetNumAtoms()
        strain_per_atom = strain / max(n_atoms, 1)

        return {
            "strain_energy": strain,
            "strain_energy_per_atom": strain_per_atom,
            "ff_name": ff_name,
        }
