...
from ..rl_objectives.partition_history_only import build_partition_selector
...
class LIFT_Lightning(RL_Lightning):
    ...
    def FM_training_step(self, batch):
        ...
        for k, v in scoring_ref.component_scores.items():
            self._log_debug(f"train-mpo-ref-debug-comp-{k}-mean", v.mean(), on_step=True, logger=True, sync_dist=True)
            self.log(f"train-mpo-ref-comp-{k}-mean", v.mean(), on_step=True, on_epoch=True, logger=True, sync_dist=True)
            self.log(f"train-mpo-ref-comp-{k}-max", v.max(), on_step=True, logger=True, sync_dist=True)

        for k, v in scoring_ref.raw_properties.items():
            self._log_debug(f"train-mpo-ref-debug-raw-{k}-mean", v.mean(), on_step=True, logger=True, sync_dist=True)
            self.log(f"train-mpo-ref-raw-{k}-mean", v.mean(), on_step=True, on_epoch=True, logger=True, sync_dist=True)
            self.log(f"train-mpo-ref-raw-{k}-min", v.min(), on_step=True, logger=True, sync_dist=True)
            self.log(f"train-mpo-ref-raw-{k}-max", v.max(), on_step=True, logger=True, sync_dist=True)
        ...
        # TPSA/logP are already logged by the generic raw_properties loop above.
        # Do not log train-mpo-ref-raw-TPSA-mean or train-mpo-ref-raw-logP-mean again;
        # Lightning forbids logging the same key twice with different on_epoch settings.
        ...
        if scoring_cur is not None:
            ...
            if quality_metrics_cur is not None:
                ...
                for k, v in scoring_cur.component_scores.items():
                    self.log(f"train-mpo-current-comp-{k}-mean", v.mean(), on_step=True, on_epoch=True, logger=True, sync_dist=True)
                    self.log(f"train-mpo-current-comp-{k}-max", v.max(), on_step=True, logger=True, sync_dist=True)
                for k, v in scoring_cur.raw_properties.items():
                    self.log(f"train-mpo-current-raw-{k}-mean", v.mean(), on_step=True, on_epoch=True, logger=True, sync_dist=True)
                    self.log(f"train-mpo-current-raw-{k}-min", v.min(), on_step=True, logger=True, sync_dist=True)
                    self.log(f"train-mpo-current-raw-{k}-max", v.max(), on_step=True, logger=True, sync_dist=True)
                ...
                # TPSA/logP are already logged by the generic current raw_properties loop above.
                # Avoid duplicate Lightning log keys with different arguments.
                for key in [
                    "strain-energy-mean",
