...
from .rl_grpo_surrogate_diff import RL_GRPO_Surrogate_Lightning as GRPO_Lightning
from .rl_ddpo_diff import RL_GRPO_Surrogate_Lightning as DDPO_Lightning
...
class MolGen_RLGRPOSurrogateModel(MolGen_Model):
    def __init__(
        ...
        **kwargs,
        ...
        if rl_mode == "grpo":
            self.lightning_cls = GRPO_Lightning
        elif rl_mode == "ddpo":
            self.lightning_cls = DDPO_Lightning
        else:
    ...
    def create_lightning_module(self, hparams=None, load_ckpt=None):
        ...
        if load_ckpt is not None:
            lightning_module = self.lightning_cls.load_from_checkpoint(
                load_ckpt,
                gen=self.network,
                vocab=self.vocab,
                map_location="cpu",
            ...
            lightning_module = self.lightning_cls(
                gen=self.network,
                vocab=self.vocab,
                **default_hparams,
            )
