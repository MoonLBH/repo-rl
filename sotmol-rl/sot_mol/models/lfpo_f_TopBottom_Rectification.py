...
class LFPOF_Lightning(RL_Lightning):
    ...
    Core design:
      - Discrete heads (atomics / bonds / charges): top/bottom implicit target rectification
        with soft cross-entropy. Top samples use the positive implicit target, bottom
        samples use the negative implicit target.
      - Continuous head (coords): top/bottom target rectification in prediction space
        with masked MSE. Top samples use the positive coordinate target, bottom
        samples use the negative coordinate target.
    ...
    def __init__(
        ...
        *args,
        ...
        # LFPO-F-v2: top-reward positive rectification + bottom-reward negative rectification
    ...
    def _reward_to_top_bottom_masks(self, rewards: torch.Tensor):
        """Return boolean masks for top-reward positive rectification and bottom-reward negative rectification.

        Top samples use LFPO-F positive implicit targets.
        Bottom samples use LFPO-F negative implicit targets.
        ...
        n_top = max(1, min(n, int(math.ceil(n * max(0.0, self.lfpo_top_ratio)))))
        n_bottom = max(1, min(n, int(math.ceil(n * max(0.0, self.lfpo_bottom_ratio)))))
        ...
        # Avoid overlap when batch is very small. Top wins.
    ...
    def FM_training_step(self, batch):
        ...
        # objective below uses hard top/bottom masks: top -> positive target, bottom -> negative target.
        ...
        if self.lfpo_log_current_reward and (
            ...
            with torch.no_grad():
                generated_cur = self._generate_with_model(
                    self.gen,
                    noise,
                    inference_steps=self.max_steps,
                    coord_noise_std=self.default_coord_noise_std,
            ...
            rewards_cur, generated_mols_cur = self._compute_rewards_from_generated(generated_cur)
            quality_metrics_cur = self._compute_generation_quality_from_mols(
                generated_mols_cur,
                dtype=generated_cur["coords"].dtype,
                device=generated_cur["coords"].device,
            )
        ...
        pos_rectification_loss = torch.zeros((), device=device, dtype=dtype)
        ...
        for k in range(self.lfpo_num_time_samples):
            ...
            for start in range(0, batch_size, batch_chunk):
                ...
                # ----------------------------------------------------------
                # LFPO-F-v2 rectification branches.
                # Top-reward samples use the positive implicit targets.
                # Bottom-reward samples use the negative implicit targets.
                # Middle samples are ignored in the main loss when
                # lfpo_middle_weight == 0.
                # ----------------------------------------------------------
                ...
                # Optional auxiliary FM target on the same generated pseudo-targets.
                # This is only used when lfpo_aux_fm_weight > 0; it is not the main
                # top-reward objective in this version.
                if self.formulation == "endpoint":
                ...
                # Positive branch = LFPO-F positive rectification.
                pos_type_ps = type_plus
                pos_bond_ps = bond_plus
                pos_charge_ps = charge_plus
                pos_coord_ps = coord_plus
                ...
                type_rect_ps = top_chunk * pos_type_ps + self.lfpo_bottom_repulsion_weight * bottom_chunk * type_minus + middle_chunk * mid_type_ps
                bond_rect_ps = top_chunk * pos_bond_ps + self.lfpo_bottom_repulsion_weight * bottom_chunk * bond_minus + middle_chunk * mid_bond_ps
                coord_rect_ps = top_chunk * pos_coord_ps + self.lfpo_bottom_repulsion_weight * bottom_chunk * coord_minus + middle_chunk * mid_coord_ps
                charge_rect_ps = top_chunk * pos_charge_ps + self.lfpo_bottom_repulsion_weight * bottom_chunk * charge_minus + middle_chunk * mid_charge_ps
                ...
                pos_rectification_loss = pos_rectification_loss + chunk_pos.detach()
        ...
        self.log("train-lfpof-pos-rect-loss", pos_rectification_loss, on_step=True, logger=True, sync_dist=True)
        ...
        if rewards_cur is not None:
            self.log("train-lfpof-reward-current-mean", rewards_cur.mean(), on_step=True, logger=True, sync_dist=True)
            self.log("train-lfpof-reward-current-max", rewards_cur.max(), on_step=True, logger=True, sync_dist=True)
            self.log("train-lfpof-reward-current-top10-mean", self._top_frac_mean(rewards_cur, 0.1), on_step=True, logger=True, sync_dist=True)
