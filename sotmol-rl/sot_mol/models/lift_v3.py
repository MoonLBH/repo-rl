# =============================================================================
# 与 lift_v2.py 的主要区别说明
# =============================================================================
# 本文件是在 lift_v2.py 的 LFPO-F / top-bottom 训练框架基础上，
# 额外加入了 SimilarityArchive（相似性样本缓存与回放）机制。
#
# lift_v2.py 的训练样本只来自当前 batch 中由 ref_gen/reference policy
# 生成的分子：每一步生成一批样本，计算 reward/MPO score 后，根据
# partition_selector 划分 top / bottom / middle，其中 top 样本用于正向模仿
# imitation，bottom 样本用于负向 repulsion，middle 样本通常不参与主损失。
#
# 本文件新增的 archive 逻辑会在每个训练 step 中，从当前生成样本里筛选
# 指定相似性指标较高的样本，例如 raw sim_ranolazine_AP >= archive_min_value，
# 将其对应的 generated target batch 保存到历史缓存池中。后续训练时，再从
# archive 中取出一部分历史高相似样本，与当前 batch 的样本拼接在一起参与训练。
#
# 被 archive 回放的样本会被强制视为 top imitation 样本：
#   - top_mask = True
#   - bottom_mask = False
#   - selected_mask = True
#   - top_imitation_w = archive_top_weight
# 因此 archive 样本只增强正向模仿信号，不参与 bottom repulsion。
#
# 该机制主要用于 Ranolazine_MPO 这类目标相似性样本极稀疏的任务。
# 当 prior/reference policy 几乎生成不出 sim >= 0.7 的分子时，即使偶然生成
# 出相似性略高的样本，也容易在下一步训练中丢失。SimilarityArchive 会把这些
# “阶段性较优”的相似性样本保留下来，并在后续反复用于 top imitation，从而提供
# 更稳定的相似性优化方向。
#
# 需要注意：
# 1. archive 默认只根据指定 key（如 sim_ranolazine_AP）筛选样本，本质上是
#    similarity-priority replay，不等价于完整 MPO 最优样本回放。
# 2. archive 样本来自历史模型分布，可能存在一定 off-policy 偏差。
# 3. 如果 archive_sample_size 或 archive_mix_ratio 过大，模型可能过度模仿少数
#    历史高相似样本，导致多样性下降。
# 4. archive_min_value 较低时，它更像 warm-start / curriculum 机制，
#    不能说明样本已经真正满足 Ranolazine_MPO 的高相似性阈值。
# =============================================================================
import copy
import math
