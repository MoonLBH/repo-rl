...
class RL_Lightning(SC_Lightning):
    ...
    def _compute_rewards_from_generated(self, generated):
        """Generate RDKit mols, evaluate the configured objective, and return rewards.

        This replaces the original hard-coded QED reward path. Any objective
        registered in sot_mol.rl_objectives.mpo_tasks.build_objective can now be
        used by passing objective_name/objective_config to RL_Lightning.
        """
        mols = self._generate_mols(generated, sanitise=True)
        scoring = self.objective.score_mols(
            mols,
            dtype=generated["coords"].dtype,
            device=generated["coords"].device,
        )
        return scoring.score, mols, scoring

    def _compute_rewards_from_mols(self, mols, dtype, device):
        """Compatibility wrapper for legacy callers."""
        scoring = self.objective.score_mols(mols, dtype=dtype, device=device)
    ...
    def FM_training_step(self, batch):
        ...
        rewards, generated_mols, scoring = self._compute_rewards_from_generated(generated)
        ...
        self.log("train-rl-objective-score-mean", scoring.score.mean(), on_step=True, on_epoch=True, logger=True, sync_dist=True)
        self.log("train-rl-objective-score-max", scoring.score.max(), on_step=True, logger=True, sync_dist=True)
        for k, v in scoring.component_scores.items():
            self.log(f"train-rl-comp-{k}-mean", v.mean(), on_step=True, on_epoch=True, logger=True, sync_dist=True)
            self.log(f"train-rl-comp-{k}-max", v.max(), on_step=True, logger=True, sync_dist=True)
        for k, v in scoring.raw_properties.items():
            self.log(f"train-rl-raw-{k}-mean", v.mean(), on_step=True, on_epoch=True, logger=True, sync_dist=True)
            self.log(f"train-rl-raw-{k}-min", v.min(), on_step=True, logger=True, sync_dist=True)
            self.log(f"train-rl-raw-{k}-max", v.max(), on_step=True, logger=True, sync_dist=True)
