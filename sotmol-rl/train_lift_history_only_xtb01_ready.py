...
parser.add_argument("--lfpo_eval_current_every", type=int, default=100,
                    help="Current-policy evaluation interval. For xTB, use a larger value or disable current eval.")
parser.add_argument("--lfpo_eval_current_samples", type=int, default=1000,
                    help="Number of current-policy samples for low-frequency evaluation. For xTB, use 32-128.")
parser.add_argument("--lfpo_eval_current_batch_size", type=int, default=256,
                    help="Chunk size for current-policy evaluation.")
parser.add_argument("--disable_lfpo_current_eval", action="store_true",
                    help="Disable current-policy reward evaluation. Useful for expensive xTB rewards.")
...
elif obj_lower in ["xtb", "xtb_force", "xtb_stability", "gfn2_xtb", "gfn2_xtb_force", "xtb_force_rms"]:
    objective_config = {
...
if obj_lower in ["qed_sa", "qed_sa_min", "qedsa", "qed_sa_mpo"]:
    ...
    # Higher reward means lower xTB force RMS because score = -force_rms.
    top_selection_component_weights = {"xtb_reward": 1.0}
elif obj_lower in ["celecoxib_similarity", "celecoxib_sim", "celecoxib_scaffold_hopping"]:
...
no_numf_floor_objectives = {
    ...
    "xtb", "xtb_force", "xtb_stability", "gfn2_xtb", "gfn2_xtb_force", "xtb_force_rms",
}
...
model = MolGen_LFPOModel(
    ...
    lfpo_hparams={
        ...
        "lfpo_eval_current_every": int(args.lfpo_eval_current_every),
        "lfpo_log_current_reward": not bool(args.disable_lfpo_current_eval),
        "lfpo_eval_current_samples": int(args.lfpo_eval_current_samples),
        "lfpo_eval_current_batch_size": int(args.lfpo_eval_current_batch_size),
    },
)
